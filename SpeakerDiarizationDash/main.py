...
class WhisperTranscriber:
    ...
    def transcribe(self, waveform):
        ...
        print(f"len(waveform.data) = {len(waveform.data)} {len(waveform.data)/16000}")
        audio = waveform.data.astype("float32").reshape(-1)
        audio = whisper.pad_or_trim(audio)

        # Transcribe the given audio while suppressing logs

        print(f" initial_prompt = {len(self._buffer)}")
        print(f" initial_prompt = {self._buffer}")
    ...
    def identify_speakers(self, transcription, diarization, time_shift):
        ...
        for segment in transcription["segments"]:
            ...
            print(f"Start = {start} End = {end} time_shift = {time_shift}")
        ...
        print(f"speaker_captions = {speaker_captions}")
...
if __name__ == "__main__":
    ...
    HfFolder.save_token('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')
    ...
    config = PipelineConfig(
        ...
        rho_update=0.1, # only use speech longer than 100ms per speaker for self-improvement
        delta_new=0.57 # new speaker detection. The lower the value, the more sensitive the system will be to differences in voices
    )
